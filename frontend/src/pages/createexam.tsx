...
const CreateExam: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:5000/api/exam/create', formData);
    ...
    return (
        <Container maxWidth="lg" sx={{marginTop: '20px', marginBottom: '30px'}}>
            ...
            <Typography variant="body2" gutterBottom sx={{ color: 'black', textAlign: 'justify', marginBottom: '20px' }}>This section is available only to teachers. Please fill in all fields. If an exam number already exists, do not try to put the same number. If you have common sense, do not put random exam numbers (eg: if exam 1 is the lalst created, do not create exam 3 without creating exam 2). The time is in second: be careful and convert the hours to seconds. Friendly reminder that 1 hours = 3600 seconds. You just have to enter 3600, it's pretty straightforward, but I had to explain just in case. Oh, and yes, if you create an exam, all of the questions that were created belonging to that exam number will be used to generate this exam... No need to create 200 questions here!</Typography>
